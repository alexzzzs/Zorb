internal static partial class Program
{
        ...
        string inputPath)
    {
        ...
        using (var document = System.Text.Json.JsonDocument.Parse(execution.StdOut))
        {
            var call = document.RootElement.GetProperty("functions")[1]
                .GetProperty("blocks")[0].GetProperty("instructions")[0];
            if (call.GetProperty("op").GetString() != "call" ||
                call.GetProperty("callee").GetInt64() != 1 ||
                call.GetProperty("arguments")[0].GetInt64() != 1 ||
                call.GetProperty("arguments")[1].GetInt64() != 2)
        ...
        string inputPath)
    {
        ...
        using (var document = System.Text.Json.JsonDocument.Parse(execution.StdOut))
        {
            ...
            var operations = instructions.EnumerateArray()
                ...
                .ToArray();
            if (!operations.SequenceEqual(["alloca", "binary", "store", "load"], StringComparer.Ordinal) ||
                instructions[2].GetProperty("lhs").GetInt64() != 3 ||
                instructions[2].GetProperty("rhs").GetInt64() != 4 ||
                instructions[3].GetProperty("lhs").GetInt64() != 3)
        ...
        string inputPath)
    {
        ...
        using (var document = System.Text.Json.JsonDocument.Parse(execution.StdOut))
        {
            var instructions = document.RootElement.GetProperty("functions")[0]
                .GetProperty("blocks")[0].GetProperty("instructions");
            if (instructions.GetArrayLength() != 4 ||
                instructions[2].GetProperty("op").GetString() != "integer_constant" ||
                instructions[2].GetProperty("integer").GetInt64() != 0 ||
                instructions[3].GetProperty("binary_op").GetString() != "sub" ||
                instructions[3].GetProperty("lhs").GetInt64() != 4 ||
                instructions[3].GetProperty("rhs").GetInt64() != 3)
        ...
        string inputPath)
    {
        ...
        using (var document = System.Text.Json.JsonDocument.Parse(execution.StdOut))
        {
            ...
            var entryTerminator = blocks[0].GetProperty("terminator");
            if (blocks.GetArrayLength() != 3 ||
                blocks[0].GetProperty("instructions")[1].GetProperty("compare_op").GetString() != "signed_greater" ||
                entryTerminator.GetProperty("op").GetString() != "conditional_branch" ||
                entryTerminator.GetProperty("condition").GetInt64() != 4 ||
                entryTerminator.GetProperty("true_target").GetInt64() != 2 ||
                entryTerminator.GetProperty("false_target").GetInt64() != 3 ||
        ...
        var llvm = File.ReadAllText(llvmPath);
        if (!llvm.Contains("icmp sgt i32 %value, 0", StringComparison.Ordinal) ||
            !llvm.Contains("br i1", StringComparison.Ordinal) ||
            !llvm.Contains("then:", StringComparison.Ordinal) ||
