...
The Linux publisher detects the native host architecture. It builds with
`host-linux` on x86_64 and `host-linux-aarch64` on AArch64, then performs the
same generation-2/generation-3 comparison on that host. Native AArch64
publishing uses the host `gcc` and executes its smoke-test binaries directly;
it does not require an AArch64 cross-compiler or QEMU.
